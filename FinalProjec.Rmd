...
year_ace_named_wa <- subset(year_ace, year_ace$basin == "Western Atlantic" & year_ace$max_category >= 3 & year_ace$year > 1975)

year_ace_hurr_wa <- subset(year_ace, year_ace$basin == "Western Atlantic" & year_ace$max_category >= 1 & year_ace$year > 1975)

year_ace_major_wa <- subset(year_ace, year_ace$basin == "Western Atlantic" & year_ace$max_category >= 3 & year_ace$year > 1975)

year_ace_intense_wa <- subset(year_ace, year_ace$basin == "Western Atlantic" & year_ace$max_category >= 4 & year_ace$year > 1975)

hurr_meta_named_wa <- subset(hurr_meta, hurr_meta$basin == "Western Atlantic"  & hurr_meta$year >= 1975 & !is.na(hurr_meta$ace) )

hurr_meta_hurr_wa <- subset(hurr_meta, hurr_meta$basin == "Western Atlantic" & hurr_meta$max_category >= 1 & hurr_meta$year >= 1975)
  
hurr_meta_major_wa <- subset(hurr_meta, hurr_meta$basin == "Western Atlantic" & hurr_meta$max_category >= 3 & hurr_meta$year >= 1975)

hurr_meta_intense_wa <- subset(hurr_meta, hurr_meta$basin == "Western Atlantic" & hurr_meta$max_category >= 4 & hurr_meta$year >= 1975)
```

```{r, fig.cap=paste("figure 1. ACE score for storms occuring 1975-1980")}

ggplot(hurr_meta_named_wa, aes(year, ace)) +
  ...
  scale_color_manual(breaks = c("1", "2", "3", "4", "5"),values=c("yellow", "yellow", "yellow", "orange", "red")) + 
  labs(size = "ACE Score", color = "Category" ) +
  ggtitle("Named Storms and ACE score by Year") +
  coord_equal() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1) , aspect.ratio=6/12, legend.position="bottom")

ggplot(hurr_meta_major_wa, aes(year, ace)) +
  geom_smooth(aes(x=year, y=ace, group = 1) , method = "lm",color="#008fd5",se=0) +
  geom_point(aes(size=ace, color=as.character(max_category)), alpha=3/4) +
  scale_color_manual(breaks = c("3", "4", "5"),values=c("yellow", "orange", "red")) +
  labs(size = "ACE Score", color = "Category" ) +
    ggtitle("Major Hurricanes and ACE score by Year") +
  coord_equal() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1) , aspect.ratio=6/12, legend.position="bottom")
...
ggplot(hurr_meta_intense_wa, aes(year, ace)) +
  ...
  labs(size = "ACE Score", color = "Category" ) +
    ggtitle("Intense Hurricanes and ACE score by Year") +
  coord_equal() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1) , aspect.ratio=6/12, legend.position="bottom")

#grid.arrange(p1, p2, p3, ncol = 1, nrow = 3,  widths=1:1)
...
```{r, fig.cap=paste("figure 1. Total ACE score by year 1975-1980")}
   ...
   ggplot(year_ace_intense_wa, aes(year, ace)) +
     geom_smooth(aes(x=year, y=ace, group = 1) , method = "lm",color="#008fd5",se=0) +
     geom_point(aes(size=ace, color=as.character(max_category)), alpha=3/4) +
    scale_color_manual(breaks = c("3", "4", "5"),values=c("yellow", "orange", "red")) +
     ggtitle("Total ACE score by year") +
     coord_equal() +
     theme_bw() +
...
```{r, fig.cap=paste("figure 2. Max Wind M/S by intense storms 1975-1980")}
 ...
 ggplot(hurr_meta_intense_wa, aes(year, log(max_wind_ms))) +
    ...
    coord_equal() +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1) , aspect.ratio=6/12, legend.position="none")
...
ggplot(year_ace_intense_wa, aes(year, log(max_wind_ms))) +
     geom_smooth(aes(x=year, y=log(max_wind_ms), group = 1) , method = "lm",color="#008fd5",se=0) +
     geom_point(aes(size=log(max_wind_ms), color=as.character(max_category)), alpha=3/4) +
    scale_color_manual(breaks = c("3","4", "5"),values=c("yellow","orange","red")) +
     ggtitle("Max wind as M/S by year") +
     coord_equal() +
     theme_bw() +
     theme(axis.text.x = element_text(angle = 90, hjust = 1) , aspect.ratio=6/12, legend.position="bottom")
```
...
```{r, fig.cap=paste("figure 1. Total ACE score by year 1975-1980")}
   ...
   ggplot(year_ace_major_wa, aes(year, log(max_wind_ms))) +
     geom_smooth(aes(x=year, y=log(max_wind_ms), group = 1) , method = "lm",color="#008fd5",se=0) +
     geom_point(aes(size=log(max_wind_ms), color=as.character(max_category)), alpha=3/4) +
    scale_color_manual(breaks = c("3","4", "5"),values=c("yellow","orange","red")) +
     ggtitle("Max wind as M/S by year") +
